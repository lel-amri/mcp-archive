...
echo "MCP $MCPVERSION running in $MCPDIR"
  ...
  python $MCPOBFUSC -c $MCPCONFDIR/client_obfuscation.txt -d $MCREOBSCRIPT -i $MCBIN -o $MCREOBDIR >> $MCPREOBLOG 2>/dev/null

  echo "+ Obfuscating server:"
  echo "+ Obfuscating server." >> $MCPREOBLOG
  cat $MCPCONFDIR/server_obfuscation.txt
  python $MCPOBFUSC -c $MCPCONFDIR/server_obfuscation.txt -d $MCSREOBSCRIPT -i $MCSBIN -o $MCSREOBDIR >> $MCPREOBLOG 2>/dev/null
